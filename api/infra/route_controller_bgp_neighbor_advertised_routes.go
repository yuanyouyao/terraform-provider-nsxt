...
type RouteControllerBgpNeighborAdvertisedRoutesClientContext utl.ClientContext

func NewRouteControllerBgpNeighborAdvertisedRoutesClient(sessionContext utl.SessionContext, connector vapiProtocolClient_.Connector) *RouteControllerBgpNeighborAdvertisedRoutesClientContext {
	var client interface{}

	switch sessionContext.ClientType {
	case utl.Local:
		client = client0.NewAdvertisedRoutesClient(connector)
	default:
		return nil
	}
	return &RouteControllerBgpNeighborAdvertisedRoutesClientContext{Client: client, ClientType: sessionContext.ClientType, ProjectID: sessionContext.ProjectID, VPCID: sessionContext.VPCID}
}

func (c RouteControllerBgpNeighborAdvertisedRoutesClientContext) List(routeControllerIdParam string, neighborIdParam string, countParam *int64, cursorParam *string, enforcementPointPathParam *string, includedFieldsParam *string, neighborAddressParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model0.RouteControllerBgpNeighborRoutesListResult, error) {
	var obj model0.RouteControllerBgpNeighborRoutesListResult
	var err error

	switch c.ClientType {
	case utl.Local:
		client := c.Client.(client0.AdvertisedRoutesClient)
