...
var cliRouteControllerForwardingTableClient = infra.NewRouteControllerForwardingTableClient

func dataSourceNsxtPolicyRouteControllerForwardingTable() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceNsxtPolicyRouteControllerForwardingTableRead,
		...
		Schema: map[string]*schema.Schema{
			...
			"forwarding_table": {
				...
				Computed:    true,
				Description: "The forwarding table entries from the route controller.",
				Elem: &schema.Resource{
					Schema: map[string]*schema.Schema{
						...
						"status": {
							Type:        schema.TypeString,
							Computed:    true,
							Description: "Status of the forwarding table retrieval.",
						},
						"transport_node_path": {
							Type:        schema.TypeString,
							Computed:    true,
						...
						"route_entries": {
							Type:        schema.TypeList,
							Computed:    true,
							Description: "List of forwarding entries.",
							Elem: &schema.Resource{
								Schema: map[string]*schema.Schema{
									"admin_distance": {
										Type:        schema.TypeInt,
									...
									"route_type": {
										Type:        schema.TypeString,
										Computed:    true,
										Description: "Route type in forwarding table.",
									},
								},
							},
						},
			...
			"forwarding_table_csv": {
				Type:        schema.TypeString,
				Computed:    true,
				Description: "The forwarding table in CSV format downloaded from the route controller.",
			},
		},
	}
}

func dataSourceNsxtPolicyRouteControllerForwardingTableRead(d *schema.ResourceData, m interface{}) error {
	...
	client := cliRouteControllerForwardingTableClient(sessionContext, connector, clients.Host, clients.PolicyHTTPClient, username, password, token, cookie, xsrf)
	if client == nil {
	...
	if err != nil {
		return fmt.Errorf("error getting route controller forwarding table for ID %s: %v", routeControllerID, err)
	}

	forwardingTables := make([]map[string]interface{}, 0, len(res.Results))
	for _, rt := range res.Results {
		...
		forwardingTables = append(forwardingTables, item)
	}
	d.Set("forwarding_table", forwardingTables)

	csvData, err := client.Download(routeControllerID, networkPrefix, routeSource, vnaPath)
	if err != nil {
		return fmt.Errorf("error downloading route controller forwarding table for ID %s: %v", routeControllerID, err)
	}
	d.Set("forwarding_table_csv", csvData)
