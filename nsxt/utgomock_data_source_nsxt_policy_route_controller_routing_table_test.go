...
func TestMockDataSourceNsxtPolicyRouteControllerRoutingTableRead(t *testing.T) {
	...
	mockRoutingTableSDK := rtmocks.NewMockRoutingTableClient(ctrl)
	...
	httpClient := &http.Client{
		Transport: mockRoundTripper(func(req *http.Request) (*http.Response, error) {
			assert.Contains(t, req.URL.Path, "/infra/route-controllers/rc-1/routing-table/download")
	...
	mockWrapper := &cliinfra.RouteControllerRoutingTableClientContext{
		Client:     mockRoutingTableSDK,
		ClientType: utl.Local,
		Host:       "localhost",
		HTTPClient: httpClient,
	}

	originalCli := cliRouteControllerRoutingTableClient
	defer func() { cliRouteControllerRoutingTableClient = originalCli }()
	cliRouteControllerRoutingTableClient = func(sessionContext utl.SessionContext, connector client.Connector, host string, hc *http.Client, username, password, token, cookie, xsrf string) *cliinfra.RouteControllerRoutingTableClientContext {
		return mockWrapper
	}
	...
	t.Run("success", func(t *testing.T) {
		...
		mockRoutingTableSDK.EXPECT().Get(rcID, &networkPrefix, &routeSource, &vnaPath).Return(model.RouteControllerRoutingTableListResult{
		...
		ds := dataSourceNsxtPolicyRouteControllerRoutingTable()
		...
		err := dataSourceNsxtPolicyRouteControllerRoutingTableRead(d, mockClients)
		require.NoError(t, err)

		assert.Equal(t, rcID, d.Id())
		assert.Equal(t, csvContent, d.Get("routing_table_csv"))

		routingTableList := d.Get("routing_table").([]interface{})
		require.Len(t, routingTableList, 1)
		rt := routingTableList[0].(map[string]interface{})
