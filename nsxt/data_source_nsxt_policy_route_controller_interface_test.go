...
func TestAccDataSourceNsxtPolicyRouteControllerInterface_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNsxtPolicyRouteControllerInterfaceDataSourceTemplate(name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet(testDataSourceName, "id"),
					resource.TestCheckResourceAttrSet(testDataSourceName, "path"),
					resource.TestCheckResourceAttr(testDataSourceName, "display_name", "iface-test"),
					resource.TestCheckResourceAttr(testDataSourceName, "mtu", "1500"),
				),
			},
		},
	})
...
func testAccNsxtPolicyRouteControllerInterfaceDataSourceTemplate(name string) string {
